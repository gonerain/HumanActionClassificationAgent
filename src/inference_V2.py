...
from model_trainer_V2 import BiLSTMAttentionClassifier
...
class ActionInferenceV2:
    ...
    def __init__(self, model_path: str, model_name: str = "yolov8n", conf: float = 0.5, window_size: int = 30):
        ...
        self.model = BiLSTMAttentionClassifier(input_size=input_size)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run real-time action inference V2")
    parser.add_argument("--model", required=True, help="Path to trained model_v2.pt")
    parser.add_argument("--video", default=0, help="Video source (int or file path)")
    args = parser.parse_args()

    infer = ActionInferenceV2(model_path=args.model)
    infer.run(video_source=int(args.video) if str(args.video).isdigit() else args.video)
